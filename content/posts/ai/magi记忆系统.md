...
L2 是从 L1 事件中派生出来的结构化理解层。L1 负责记录**发生过什么**，L2 负责整理**这些事情说明了什么**。

它不是原始事实层，而是**有证据支持的解释层**。这意味着 L2 里的任何结论都不能脱离 L1 证据单独存在。
...
而从实现视角来看，L2 不是直接保存一段“理解后的文字”，而是先把事件里值得长期追踪的要素抽成实体。这里的实体不只指人，也可以是项目、文章、仓库、地点、主题等。
