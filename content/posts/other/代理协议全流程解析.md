...
由于近期代理流量规则发生了显著变化，主流代理服务频繁进行协议切换。加上近期社区里有个匿名作者建了个 [VLESS-cracker](https://github.com/Anonymous376c1d0cf28/VLESS-cracker) 仓库，声称发现 REALITY 协议的 100% 精准探测漏洞，issue 区吵翻了天。这两件事让我发现，用代理工具十多年了，自己其实从没仔细看过各种代理协议到底是怎么设计的，平时都是照教程配。

网上的许多博文大多停在"怎么配 + 一句话原理"这层，对机制本身讲得比较浅。在一通翻阅源码、论文、issue + AI 辅助下，写出了本文。
...
- **JA3 / JA4 指纹**：把 ClientHello 里若干固定字段拼起来算的哈希，用来识别"这是哪个客户端栈发出来的握手"。Go/Python/Chrome/Firefox 各有各的 JA3。这意味着**即使外层握手字节看起来是一个普通 HTTPS 站点的握手，只要客户端用的是 Go/Rust/Python 自己的 TLS 库，握手指纹也跟真浏览器对不上**——这是 Trojan / ShadowTLS / Reality 这一系都要面对的隐性问题。
- **uTLS / 客户端栈伪装**：解决上一条问题的两条路线。轻量版是 [uTLS](https://github.com/refraction-networking/utls)——一个 Go 库，专门把 ClientHello 字段重排成 Chrome / Firefox / Safari 的样子，让 JA3/JA4 跟真浏览器对得上；后续会讲到的 V2Ray / Xray 主线在 2022 年才接上 uTLS。重量版直接复用真浏览器整个网络栈，代表是 NaiveProxy（详见后面章节）。
...
flowchart TD
    ...
    B --> F[TLS 伪装路线]
    F --> NV[NaiveProxy 2018<br/>复用 Chromium 网络栈<br/>真浏览器 TLS+HTTP/2 指纹]
...
- **包长分布固定**。每个 AEAD chunk 的结构是 `[2字节加密长度][长度 Tag (16字节)][密文 payload][payload Tag (16字节)]`，两个 Tag 是分别针对长度字段和 payload 字段的独立 GCM 认证标签。整体结构高度规整，连续抓几个包就能看出规律。
- **服务器对任何 TCP 连接都"愿意解密"**。探测方可以主动连过来发一段随机数据，观察服务器的响应模式（是直接关闭、还是等超时、还是回错误）来推断这是不是一个 SS 服务器——这就是**主动探测**。
- **流量汇聚**。一个境外 IP 上，长期出现来自同一个客户端的、大量"看起来不像什么"的高熵 TCP 连接。从流量结构上看就是一个非常异常的流量画像。
...
从流量上来看，看到的是：真 TLS 1.3 握手、合法证书、正常 Web 流量模式。内层那个"全随机字节流"根本看不到，被外层 TLS 包了一层。
...
- **传输层**（怎么把这些字节运过公网，决定流量外观）：TCP、WebSocket、gRPC、HTTPUpgrade、xhttp、mKCP、QUIC 等可以自由组合
...
2. **在加密隧道里跟踪内层流量是不是 TLS 1.3**: 看内层 ServerHello 里的 `supported_versions` 扩展是不是 0x0304，再看协商出来的 cipher 是不是合规的 TLS 1.3 cipher suite。**只有这两条都满足才会启用直传**，否则全程走普通 TLS 模式。
...
跟 V2Ray / Xray 家族的"协议层 + 传输层正交组合"思路不同，**这一族协议走的是另一条路**——协议层和伪装层一体化，整个协议**就是基于真实 TLS 来伪装**。它们解决的核心问题都是同一个：**让代理流量在线缆上看起来就是普通 HTTPS**。

家族内部其实有两条正交的子线：

- **服务端伪装线**（让主动探测时看到的服务端"像真实 HTTPS）：Trojan → ShadowTLS → AnyTLS 是这条线的三代演化
- **客户端栈伪装线**（让被动探测时看到的握手字节、HTTP/2 帧调度"跟真浏览器一致）：NaiveProxy 是这条线的代表

四种思路按演化时间排开：

| 协议 | 年份 | 核心思路 | 一句话总结 |
|---|---|---|---|
| **NaiveProxy** | 2018 | 客户端直接用 Chromium 网络栈发请求，服务端是真 Caddy/HAProxy，代理流量走 HTTP/2 CONNECT 隧道 | 客户端就是 Chrome 本人 |
| **Trojan** | 2019 | 服务器端就是真 HTTPS 服务，握手用自己的真证书，密码错了走 fallback 站点 | 服务端不伪装，做真的 |
...
下面分别看 NaiveProxy、Trojan、ShadowTLS、AnyTLS 四个协议。

## NaiveProxy

NaiveProxy 在 2018 年开源，是 TLS 伪装家族里出现最早的一个。它走的是跟 Trojan / ShadowTLS / Reality 完全不同的路线，他们解决的是"**服务端**怎么像真站点"，NaiveProxy 解决的是"**客户端**发出来的握手字节、HTTP/2 帧、流控行为怎么跟真浏览器一模一样"。

它的实现非常激进：**客户端直接复用 Chromium 的网络栈代码**，客户端发出去的每个字节——TLS ClientHello、HTTP/2 SETTINGS、WINDOW_UPDATE、HEADERS 帧调度等，全部由 Chrome 同源代码产生，跟真 Chrome 浏览器在统计学上**完全无法区分**。这是 uTLS 这类"重写 ClientHello 字段"的轻量方案做不到的，uTLS 只能对齐握手帧字段，但 HTTP/2 帧的发送时序、流控窗口大小这些行为指纹仍然是自己的。

服务端这边走的是**"借一个真的网站"**：服务器跑的就是 [Caddy](https://github.com/caddyserver/caddy)（一个开源 web 服务器，自带反向代理能力）。代理流量伪装成普通 HTTP/2 请求——带正确密码的当代理转发，带错或者没带密码的当普通访客处理，看到的是 Caddy 自己服务的静态网站。从外部视角看，这台机器就是个跑着 Caddy 的 HTTPS 站点，没什么异常。
...
sequenceDiagram
    participant App as 浏览器/应用
    participant Local as NaiveProxy 客户端
    participant Remote as Caddy 服务端
    participant Site as 同机静态站点
    participant Target as 目标网站

    Note over Local: 直接复用 Chromium 网络栈<br/>TLS+HTTP/2 指纹与真 Chrome 一致

    App->>Local: SOCKS5 请求 example.com:443
    Local->>Remote: TLS 握手 + HTTP/2 请求（带密码头）
    Note over Local,Remote: 前 8 个数据包做 padding<br/>压平典型 HTTPS 握手包长分布

    alt 密码正确
        Remote->>Target: 建立到目标的 TCP 连接
        loop 数据转发
            Local->>Remote: 隧道内数据
            Remote->>Target: 转发
            Target-->>Remote: 响应
        ...
        Remote->>Site: 当成普通访问转给静态站点
        Site-->>Remote: 静态网页
        Remote-->>Local: 看起来就是访问了一个普通网站
    end
```

NaiveProxy 解决的核心问题是 **这是不是一个代理客户端在发包**。Trojan / ShadowTLS / Reality 都把火力放在"让服务端看起来真"，但客户端那一侧的 TLS 库指纹（JA3/JA4）、HTTP/2 流控行为对这类分类器来说仍然是清晰特征。NaiveProxy 直接复用 Chromium 网络栈，等于把这条防线一次性补到上限。

代价也很明确：

- **服务端绑死在 Caddy 上**——必须用 `xcaddy` 重新编译带 forwardproxy 插件的 Caddy，部署门槛比 VLESS+Reality 高出一档
- **客户端必须跟着 Chrome 升级**——Chrome 改了 TLS 或 HTTP/2 行为，旧版本 NaiveProxy 的指纹就跟真 Chrome 对不上，反而暴露
- **TLS-in-TLS 数据层没全程消除**——前 8 个包做 padding 只压平了开头，后续包的 TLS-in-TLS 特征仍然存在。NaiveProxy 押注的是"信息密度集中在开头、后续对分类器价值低"，跟 XTLS Vision "全程直传消除指纹"不是同一个量级
...
但代价是 **TLS-in-TLS 指纹**绕不过去。当用户用 Trojan 访问 HTTPS 网站时，外层是 Trojan 的 TLS、内层是访问目标的 TLS。前面 Vision 章节讲过的 [Trojan-killer](https://github.com/XTLS/Trojan-killer)<sup>[\[7\]](#ref-7)</sup>就是这个攻击面的可复现演示。**另外 Trojan 客户端用的是 Go / C++ 自己的 TLS 库**，ClientHello 字段排列是固定的库指纹——前面 NaiveProxy 章节讲到的"客户端栈伪装"问题在 Trojan 上同样存在，对 USENIX 那类 ML 分类器来说仍是清晰特征。社区后来出现的"Trojan + uTLS" 修补版试图把握手指纹做成 Chrome，但 HTTP/2 流控行为这一层修不到。
...
读到这里大概率会有一个疑问：**Reality 和 ShadowTLS 不都是"借真站点的 TLS 握手"吗，到底差在哪？** 它们在哲学上确实同源，所以前面的进化树和家族总表都把它们放在一起讲过，但具体落地差别很大。直接放一张对比表：

| 维度 | Reality（2023，Xray-core 内置） | ShadowTLS（2022，独立项目） |
|---|---|---|
| 谁产生握手字节 | **代理服务器自己**（在内存里用真站点元数据 + 临时私钥伪造一份证书） | **donor 站点本人**（代理服务器只是把 ClientHello / ServerHello / 证书 字节原样转发） |
| 证书是真的吗 | 假——元数据复制自真站点，私钥是代理服务器自己生成的 | 真——donor 自己签的，浏览器都认 |
| 客户端怎么验证身份 | 不验证证书。靠预共享 publicKey + ClientHello SessionID 里的 X25519 鉴权字段 | 不验证证书。靠 ClientHello SessionID 里的 PSK HMAC + 数据帧每帧 4B HMAC |
| 是否绑定特定内层协议 | 是——只能跟 VLESS 配合，是 Xray-core 的内置扩展 | 否——内层换 SS / Trojan / VLESS 任意一种都能跑，本质是个 TLS 伪装外壳 |
| 主动探测打过来的行为 | 透明转发握手到真实 dest，对方真证书 + 真响应原样回流 | 验证失败 → 退化成 SNI Proxy，持续把流量转给 donor |
| 服务端实现要不要 TLS 库 | 要——需要在内存里生成伪造证书、跑 TLS 1.3 握手 | 不要——服务端只需要按 PSK 校验 HMAC、按字节 XOR/HMAC 转发，根本不需要 TLS 库 |
| 跟真站点的关系 | dest 站点只是一个"模板"——平时不需要连过去，只在主动探测来时透明转发 | donor 是必经路径——握手期间真有一条到 donor 的 TCP 连接在跑，握手完后才断 |
| 失败时落点 | 真 dest 站点（探测者看到的就是 microsoft.com 本人） | 真 donor 站点（同上） |
| 代价 | 跟 dest 强绑定（dest 改 TLS 配置 / 被封 / 宕机就跟着崩）；只跟 VLESS 配 | 跟 donor 强绑定（同上）；内层必须自带加密；握手期 donor 的真实带宽要服务端代付 |

一句话总结：**Reality 自己产生握手字节但伪造得逼真；ShadowTLS 直接借真站点的握手字节，自己只做转发。** 前者是 "VLESS 生态内的握手伪装方案"，后者是 "通用的 TLS 伪装外壳"，两者都能让主动探测彻底失效，但实现复杂度、生态绑定程度、对真站点的依赖方式都不同。
...
下面分别看 Hysteria 和 TUIC——它们在这个共同框架上的差异化设计完全不同。
...
| NaiveProxy | 2018 | 复用 Chromium 网络栈，让客户端 TLS+HTTP/2 指纹跟真 Chrome 一致 | 客户端二进制大且需跟 Chrome 同步升级；生态边缘 |
| Trojan | 2019 | 服务端就是真 HTTPS 站点，密码错了 fallback 到真网页 | TLS-in-TLS 数据层有包长指纹；客户端 TLS 库指纹仍可识别 |
| VLESS | 2020 | 砍掉 VMess 内层加密 | 单独裸跑就是明文 |
| Hysteria | 2020 / v2: 2023 | 跨境高丢包下榨干带宽 + 真 HTTP/3 伪装 | 服务端出口流量大；UDP 被封即挂 |
| XTLS Vision | 2022 | 消除 TLS-in-TLS 的数据层包长指纹 | 仅对内层 TLS 1.3 有效；握手层指纹要别的协议配合 |
...
- NaiveProxy README & padding protocol：[klzgrad/naiveproxy](https://github.com/klzgrad/naiveproxy)
...
- [ban6cat6/Aparecium](https://github.com/ban6cat6/Aparecium)：2025-06 公开的 REALITY / ShadowTLS v3 探测工具，利用 TLS 1.3 post-handshake records 与 OpenSSL 的实现差异；作者明确说明对非 OpenSSL-based target（如 Google）无效
- [Anonymous376c1d0cf28/VLESS-cracker](https://github.com/Anonymous376c1d0cf28/VLESS-cracker)：2026-05 匿名作者发布的"VLESS / REALITY 探测 PoC"，把 Aparecium 已知差异重新包装；实测 PoC 对真实 VLESS 节点无反应、对正常网站误报。本文写作的直接触发点
...
- NaiveProxy：[klzgrad/naiveproxy](https://github.com/klzgrad/naiveproxy)（基于 Chromium 网络栈）；服务端 forwardproxy 插件：[klzgrad/forwardproxy](https://github.com/klzgrad/forwardproxy)
...
- uTLS（客户端 TLS 指纹伪装库）：[refraction-networking/utls](https://github.com/refraction-networking/utls)
